...
// Generate color map based on index
const generateColorMap = () => {
  const colorMap: { [key: string]: string } = {};
  const characters = "ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789";
  for (let i = 0; i < characters.length; i++) {
    colorMap[characters[i]] = pastelColors[i % pastelColors.length];
  }
  return colorMap;
};
...
// Function to get background color based on the first character
export function getAvatarBg(str: string = "Unknown"): string {
  // Remove spaces and '+' characters, and convert to uppercase
  const cleanedStr = str.replace(/[\s+]/g, "").toUpperCase();

  // Get the first character
  const firstChar = cleanedStr?.charAt(0);

  // Return the corresponding color or a default color if not found
  return colorMap[firstChar] || "#FFFFFF"; // Default to white if character is not found
}
...
export const getBorderStyle = () => (Platform.OS === "ios" ? "solid" : "dashed");
